...
#mergedfile = 'pedestals_280255_280396.root'
...
def plotchannel(etax, phiy, z=0):
    ...
    if z==0:
        ...
        fed,ch  = em.FedChFromDetId(eb.rawId())
 
    else:
        ee = EEDetId()
        ee.fromXYZ(etax,phiy,z)
        fed,ch  = em.FedChFromDetId(ee.rawId())
    ...
    rfile = TFile(basedir+'pedestals_fed'+str(fed)+'_'+str(runmin)+'_'+str(runmax)+'.root')
    t= rfile.Get('peds')
    ...
    if plotrms : 
        ...
        g12 = TGraph(len(times),times,means12)
