...
def makeplots(peds1,peds2,pedsrms1,pedsrms2,outprefix):
    ...
    print 'mean_diff_eb ', heb.GetMean(), 'rms_diff_eb ', heb.GetRMS(),'mean_diff_ee ', hee.GetMean(), 'rms_diff_ee ', hee.GetRMS()  
    ...
    for idx in range(kBarrelSize):
         ...
         # assume 200.0 means nothing useful
         if ebpeds1[idx]==200.0 or  ebpeds2[idx] ==200.0 : continue
         ...
         if abs(diff)    > 5 and ebpeds2[idx] < -98 and channelStatusEB(idx)==0:
    ...
    for idx in range(kEndcapSize):
         ...
         # assume 200.0 means nothing useful
         if eepeds1[idx]==200.0 or  eepeds2[idx] ==200.0 : continue
